...
4. **Approve falls through to allow (unless autonomous).** Since 2.0.17, `approve` knobs don't block — they let Claude Code's native permission prompt handle it. Exception: on the `autonomous` preset, `approve_fallback: 'block'` converts all `approve` to `block` since there's no human to approve.
...
| File | Tests | Purpose |
|------|-------|---------|
| `calibration.test.ts` | 441 | Full pipeline verification (CET → KnobGate → Engines) |
...
**Total: 715 tests across 14 files.** Do not reduce this number.
